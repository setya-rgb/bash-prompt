...
info "Installing Bash Prompt Framework → $PREFIX"
...
__prompt_git() {
    ...
    printf "%s⎇ %s%s%s " "$status_color" "$branch" "$ahead_behind" "$C_RESET"
}
EOF
...
__prompt_venv() {
    ...
    [ -n "$venv" ] && printf "%s🐍 %s%s " "$C_GREEN" "$venv" "$C_RESET"
}
EOF
...
__prompt_kubecontext() {
    if command -v kubectl &>/dev/null && [ -f "$HOME/.kube/config" ]; then
        ...
        if [ -n "$context" ]; then
            if [ -n "$namespace" ]; then
                printf "%s⎈ %s/%s%s " "$C_MAGENTA" "$context" "$namespace" "$C_RESET"
            else
                printf "%s⎈ %s%s " "$C_MAGENTA" "$context" "$C_RESET"
            fi
        fi
    fi
}
...
__prompt_status() {
    ...
    if [ $exit_code -ne 0 ]; then
        printf "%s✗ %s%s " "$C_RED" "$exit_code" "$C_RESET"
    fi
}
EOF
...
cat >"$PREFIX/bin/prompt" <<'EOF'          #!/usr/bin/env bash                                                                   PROMPT_DIR="${HOME}/.bash_prompt"          CONFIG_FILE="$PROMPT_DIR/config.sh"                                                   show_help() {
    ...
    prompt symbol '➜'                          prompt reload                              prompt fix                                                                        HELP                                       }                                          
...
PROMPT_LEFT=(user host path git)
PROMPT_RIGHT=(venv kubecontext status jobs time)
PROMPT_SYMBOL="❯"                          PROMPT_SYMBOL_ROOT="#"
PROMPT_PATH_MAX_LEN=40
PROMPT_GIT_SHOW_BRANCH=true
PROMPT_GIT_SHOW_STATUS=true
PROMPT_GIT_SHOW_AHEAD_BEHIND=true
